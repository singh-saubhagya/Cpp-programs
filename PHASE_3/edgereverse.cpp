...
void enigma()
{
    ...
    //     if(edge.find(make_pair(b,a))==edge.end())
    //     {
    //         g[b].push_back({a,1});
    //     }
    // }
    ...
    deque<pair<lli,lli>> dq;
    dq.push_front({0,1});
    dist[1]=0;
    vector<lli> rvedge(n+1,0);
    while(dq.empty()==false)
    {
        pair<lli,lli>  x=dq.front();
        dq.pop_front();
        if(marked[x.second])
        continue;
