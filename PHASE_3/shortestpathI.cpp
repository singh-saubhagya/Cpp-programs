...
signed main()
{
  ...
  while(pq.empty()==false)
  {
    ...
    for(auto z:g[x.second])// neighbours
    {
        lli dt=z.second;// weight
        lli node=z.first;// node
        lli temp=dt+abs(x.first);// distance from the x to the current node 
        if((dis[node]==-1) || temp<(dis[node]))
        {
            dis[node]=temp;
            pq.push({-temp,node});
        }
