...
//  PROBLEM:
//  Description
// You are given an integer N and a decimal number M. M can be represented as Y.Z
//  where . is the decimal point and N as X. You have to find (Z+X).(Y+X)
// 2nd approach is to take input in integer seperated by dot character
 ...
 signed main()
 {
    ...
    while (q--)
    {
        ...
        for(int i=0;i<str.size();i++)
        {
         ...
         string result=to_string(stoi(temp2)+n)+'.'+to_string(stoi(temp1)+n);
        v.push_back(result);
    }
    for(string str:v)
    {
